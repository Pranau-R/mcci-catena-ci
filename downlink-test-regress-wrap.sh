...
##############################################################################
#
# Module: downlink-test-regress-wrap.sh
#
# Function:
...
function ci_downlink-test_generic {
    ...
    for iSketch in "$@"; do
        ...
        MCCI_CI_FILTER_NAME="$(dirname "$iSketch")/extra/ci/downlink-test-filter.sh"
...
function ci_samd {
    ...
    ci_downlink-test_generic "$@"
}

function ci_stm32 {
    _boxcomment "STM32"
    typeset -a MCCI_BOARDS=(mcci_catena_4801)
    typeset -a MCCI_REGIONS=(us915 in866)
    typeset -a MCCI_RADIOS=(sx1276)
...
function _main {
    ...
    _setup_path
    _setup_downlink-test downlink-test
    _setup_arduino_cli
